  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        ...
        <div class="actions">
          <div class="search-bar">
            ...
            <input placeholder="Search Contacts..." [(ngModel)]="searchQuery" (keyup.enter)="search()">
          </div>
      ...
      <div class="card" *ngIf="!loading">
        ...
        <div *ngIf="items.length === 0" class="empty-state"><mat-icon class="empty-icon">contacts</mat-icon><h3>No Contacts found</h3><p>Get started by creating your first record.</p></div>
      </div>
    </div>
  `,
...
export class ContactListComponent implements OnInit {
  ...
  loadData() {
    this.loading = true;
    this.api.getPage('contacts', this.page, this.pageSize).subscribe({
  ...
  search() {
    ...
    this.api.getPage('contacts/search', 0, this.pageSize, { q: this.searchQuery }).subscribe({
  ...
  delete(id: string) {
    if (confirm('Are you sure?')) {
      this.api.delete('contacts/' + id).subscribe({ next: () => { this.notify.success('Deleted successfully'); this.loadData(); } });
    }
  }
}
