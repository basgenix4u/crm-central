    ...
    <div class="page-container">
      <div class="page-header">
        ...
        <div class="actions">
          <div class="search-bar">
            ...
            <input placeholder="Search Leads..." [(ngModel)]="searchQuery" (keyup.enter)="search()">
          </div>
      ...
      <div class="card" *ngIf="!loading">
        <table mat-table [dataSource]="items" class="data-table">
          ...
          <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef></th><td mat-cell *matCellDef="let r"><button mat-icon-button [matMenuTriggerFor]="menu"><mat-icon>more_vert</mat-icon></button><mat-menu #menu="matMenu"><button mat-menu-item><mat-icon>visibility</mat-icon>View</button><button mat-menu-item (click)="delete(r.id)"><mat-icon>delete</mat-icon>Delete</button></mat-menu></td></ng-container>
        ...
        <div *ngIf="items.length === 0" class="empty-state"><mat-icon class="empty-icon">person_add</mat-icon><h3>No Leads found</h3><p>Get started by creating your first record.</p></div>
      </div>
    </div>
  `,
...
export class LeadListComponent implements OnInit {
  items: any[] = [];
  displayedColumns = ['name','email','company','status','score','actions'];
  loading = true;
  totalElements = 0;
  pageSize = 20;
  page = 0;
  ...
  loadData() {
    this.loading = true;
    this.api.getPage('leads', this.page, this.pageSize).subscribe({
  ...
  search() {
    ...
    this.api.getPage('leads/search', 0, this.pageSize, { q: this.searchQuery }).subscribe({
  ...
  delete(id: string) {
    if (confirm('Are you sure?')) {
      this.api.delete('leads/' + id).subscribe({ next: () => { this.notify.success('Deleted successfully'); this.loadData(); } });
    }
  }
}
